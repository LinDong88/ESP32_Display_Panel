...
/**
 * @brief LCD driver class for ST7102 display controller
 *
 */
class LCD_ST7102: public LCD {
public:
    /**
     * @brief Default basic attributes for ST7102 LCD controller
     */
     ...
     */
    LCD_ST7102(Bus *bus, int width, int height, int color_bits, int rst_io):
     ...
     */
    LCD_ST7102(Bus *bus, const Config &config):
     ...
     * @note Vendor initialization commands may vary between manufacturers. Consult LCD supplier for them
     */
    LCD_ST7102(const BusFactory::Config &bus_config, const Config &lcd_config):
     ...
     */
    [[deprecated("Use other constructor instead")]]
    LCD_ST7102(Bus *bus, uint8_t color_bits = 0, int rst_io = -1): LCD_ST7102(bus, 0, 0, color_bits, rst_io)
    {
    }
...
/**
 * @brief Alias for backward compatibility
 * @deprecated Use `esp_panel::drivers::LCD_ST7102` instead
 */
using ESP_PanelLcd_ST7102 [[deprecated("Use `esp_panel::drivers::LCD_ST7102` instead")]] =
    esp_panel::drivers::LCD_ST7102;
